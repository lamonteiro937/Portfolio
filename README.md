# Data Scientist

#### Technical Skills: Python, SQL, Java, Apache Spark, Tableau, Alteryx, UiPath

## Education		       
- A.S., Science | University of Massachusetts Lowell (_May 2017_)
- B.S., Mathematics | Salem State University (_December 2020_)

## Work Experience
<div style="text-align: left;">
  <img src="https://assets.bizclikmedia.net/900/1f5eaa4ba8ac1dfd241a4f12652f5f64:a67ead712eb42c374a8298694212d254/picture" alt="My centered image" width="50" height="50"/>
</div>
**Data Scientist @ J.P Morgan Chase (_February 2022 - November 2024_)**
- Led cross-functional teams to drive new platform adoption, utilizing Python to address migration gaps and entitlements for 1,200+ clients, resulting in $2M savings from legacy platform decommissioning.
- Developed centralized repository for 2,000+ fee agreements and engineered fee agreement digitalization with Python in collaboration with AI team, cutting $2M in billing errors and enhancing data access.
- Performed EDA on client experience ratings across the entire business and applied machine learning (decision trees, random forests) in Python to identify key satisfaction drivers, enhancing client experience.
- Automated cost-to-serve models, fund management fees, and financial reporting with Python, Alteryx, and UiPath, reducing 300+ hours of manual work and improving client satisfaction.
- Created and deployed interactive Tableau dashboards to analyze operational costs and platform usage, improving proposal accuracy, strategic decision-making, and client experience.


## Projects
### Fast Food Marketing Campaign A/B Testing
Conducted an A/B test using the Kruskal-Wallis test in Python to analyze the impact of three different marketing campaigns for a new product at a fast-food chain. By evaluating weekly sales data from multiple locations, the most effective promotional strategy was identified, enabling the company to make data-driven decisions that maximize sales and optimize marketing spend for the new menu item.

<div style="text-align: center;">
  <img src="https://cdn.i.haymarketmedia.asia/?n=campaign-asia%2fcontent%2ftop10fastfood_main_1200.jpg&h=570&w=855&q=100&v=20170226&c=1" alt="My centered image" width="350" height="275"/>
</div>

[View source Code](https://github.com/lamonteiro937/Portfolio/blob/db69e68356a06e8c679db1177bc4a6744dcaece7/Projects/Fast_Food_Marketing_Campaign%20_AB_Testing.ipynb)

### Credit Card Fraud Detection

Developed and implemented machine learning models, including Random Forest and advanced techniques like SMOTE and SMOTE Tomek Links, to enhance credit card fraud detection. By addressing class imbalance and improving model accuracy, the solution significantly reduced false positives, better identified fraudulent transactions, and contributed to minimizing financial losses, improving customer trust, and optimizing fraud prevention efforts for the bank.

<div style="text-align: center;">
  <img src="https://cdn.prod.website-files.com/678b8171aaf08b155798f8fb/67b2c88c3ea1fd433bfc9de7_5-p-2000.png" alt="My centered image" width="500" height="375"/>
</div>

[View source Code](https://github.com/lamonteiro937/Portfolio/blob/db69e68356a06e8c679db1177bc4a6744dcaece7/Projects/Credit_Card_Fraud_Detection.ipynb)

### Netflix Recommendation Systems
Developed a recommendation system for Netflix using Python to enhance user experience and increase platform engagement. By leveraging rank-based, user-item collaborative filtering, and matrix factorization techniques, the system delivered personalized movie recommendations. This improved user satisfaction, increased the time customers spent on the platform, and contributed to higher engagement and retention rates, driving overall growth for Netflix.

<div style="text-align: center;">
  <img src="https://images.ctfassets.net/y2ske730sjqp/1aONibCke6niZhgPxuiilC/2c401b05a07288746ddf3bd3943fbc76/BrandAssets_Logos_01-Wordmark.jpg?w=940" alt="My centered image" width="350" height="300"/>
</div>

[View source Code](https://github.com/lamonteiro937/Portfolio/blob/db69e68356a06e8c679db1177bc4a6744dcaece7/Projects/Netflix_Recommendation_System.ipynb)
